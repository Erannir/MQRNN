...
class Decoder(nn.Module):
    ...
    def forward(self, hidden_states, x_f):
        ...
        """
        hidden_states = hidden_states[:, 0, :].unsqueeze(
            1)  # dimensions: (batch, 1, hidden_size) - currently until we decide how to cut, the forking (T) length is 1
        ...
        contexts = self.global_mlp(
            global_input)  # (c_t+1,...,c_t+K, c_a)      # dimensions: (batch, 1, context_size * (horizon + 1))
        ...
        c_t = c_t.view(batch_size, seq_len, self.horizon,
                       self.context_size)  # dimensions: (batch, 1, horizon, context_size)
        c_a = c_a.unsqueeze(2).expand(batch_size, seq_len, self.horizon,
                                      self.context_size)  # dimensions: (batch, 1, horizon, context_size)
        x_f = x_f.view(batch_size, seq_len, self.horizon,
                       self.embed_size)  # dimensions: (batch, 1, horizon, embed_size)
        local_input = torch.cat((c_t, c_a, x_f),
                                dim=-1)  # dimensions: (batch, 1, horizon, 2 * context_size + embed_size)
...
class MQRNN(nn.Module):
    ...
    def forward(self, y_e, x_e, x_d):
        ...
        predictions = self.decoder(hidden_states,
                                   x_d)  # dimensions: (batch, len(hidden_states), horizon, len(quantiles))
        return predictions
